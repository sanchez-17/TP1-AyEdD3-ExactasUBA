...
const int arr = 1 ;//Arriba
const int aba = 2 ;//Abajo
const int izq = 3 ;//Izquierda
const int der = 4 ;//Derecha
...
void senderos(int i, int j, int desde, int num){
    ...
    if(i == N - 1 && j == M - 1){
        MAX = MAX > num ? MAX : num;//max(MAX, num);
        MIN = MIN < num ? MIN : num;//min(MIN, num);
        posible = true;
        return;
    }else{
        camino[i][j] = true;bool a,b;
        a = b = false;
        if( (j==0 || j==M-1) && (i!=0) && desde != arr && !camino[i-1][j]) {camino[i-1][j] = true;a = true;}
        if( (i==0 || i==N-1) && (j!=0) && desde != izq && !camino[i][j-1]) {camino[i][j-1] = true;b = true;}
        if(tablero[i][j] == 'I'){
